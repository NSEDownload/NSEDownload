...
class Test_stocks(unittest.TestCase):
    ...
    def test_get_data(self):
        ...
        df_actual = pd.read_csv('tests/RELIANCE.csv', index_col='Date')
    ...
    def test_get_data_check_name(self):
        ...
        df_actual = pd.read_csv('tests/ANGELONE.csv', index_col='Date')
        assert(df.equals(df_actual))
    ...
    def test_adjusted_data(self):
        ...
        df_actual = pd.read_csv('tests/IOC.csv', index_col='Date')
        assert(df.equals(df_actual))

    def test_get_adjusted_stock(self):

        df = stocks.get_adjusted_stock(
            stockSymbol='HDFCBANK', start_date='12-1-2011', end_date='12-2-2011')

        df_actual = pd.read_csv(
            'tests/HDFCBANK.csv', index_col='Date', parse_dates=True, infer_datetime_format=True)
